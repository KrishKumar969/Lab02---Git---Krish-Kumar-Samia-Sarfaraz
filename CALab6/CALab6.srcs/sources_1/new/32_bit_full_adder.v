...
// ============================================================
//  ThirtyTwobitAddSub
//  Mode: Sub=0 ? Addition  (S = A + B)
//        Sub=1 ? Subtraction (S = A - B, using two's complement)
//
//  Trick: XOR each B bit with Sub.
//    Sub=0 ? B[i] ^ 0 = B[i]        (normal B), Cin to FA0 = 0
//    Sub=1 ? B[i] ^ 1 = ~B[i]       (invert B), Cin to FA0 = 1
//  Inverting B and adding 1 gives two's complement negation.
//
//  Overflow: OVF = C[30] ^ Cout  (signed overflow flag)
//  Cout is the final carry out (unsigned overflow / borrow indicator)
// ============================================================

module ThirtyTwobitAddSub(
    input  [31:0] A,
    input  [31:0] B,
